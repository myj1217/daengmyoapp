...
const BasicMenu = () => {
  ...
  const update = () => {
    ...
    stompClient.current.connect({}, () => {
      stompClient.current.subscribe(
        `/topic/chat/email/${loginState.email}`,
        (message) => {
  ...
  return (
    <header
      className={`flex bg-green-100 text-green-800 p-4 w-full h-20 top-0 z-50 ${
    ...
    >
          ...
          {isLogin ? (
            <div className="flex gap-4">
              ...
              >
                ...
                {cartItems.length !== 0 && (
                  <div className="mb-2 ml-0.5 text-xs rounded-full bg-red-500 w-4 h-4 flex items-center justify-center text-white">
                    {cartItems.length}
                  </div>
                )}
              </Link>
      ...
      {isLogin && (
        <>
          <button
            className="fixed z-80 right-8 bottom-96 z-50 p-2 bg-emerald-400 text-white rounded-full shadow-lg"
            onClick={() => {
