...
const ChatReportListComponent = () => {
  ...
  const fetchReportList = () => {
    jwtAxios
      .get(`${API_SERVER_HOST}/api/chat/report`)
      .then((response) => {
        const fetchedReportList = response.data.map((report) => ({
          ...report,
          sendTime: new Date(report.sendTime).toLocaleString("ko-KR", {
            timeZone: "Asia/Seoul",
          }), // 한국 표준 시간대로 변환
        }));
  ...
  const handleStatusChange = (reportId, completed) => {
    jwtAxios
      .put(
        `${API_SERVER_HOST}/api/chat/report/${reportId}?completed=${completed}`
      )
      .then((response) => {
        fetchReportList();
      ...
      <table className="w-full ">
        <thead>
          <tr className="bg-gray-200">
            ...
            <th className="border  px-4 py-2 w-[300px]">보낸 시간</th>
            <th className="border  px-4 py-2">메세지</th>
            <th
              className="border  px-4 py-2 w-[150px] cursor-pointer"
        ...
        <tbody>
          {sortedReportList.map((report) => (
            <tr
              key={report.id}
              className={report.completed ? "bg-gray-300" : ""}
            >
              ...
              >
                <select
                  ...
                  defaultValue={report.completed ? true : false}
                  className={`px-2 py-1 ${
                    report.completed ? "bg-gray-300" : ""
                  }`}
                >
